...
public class ReservationController
{
    ...
    @GetMapping("/payed")
    public ResponseEntity<List<Reservation>> getPayedReservations(Authentication authentication)
    {
        List<User> user = userService.getUser(authentication.getName(), authentication.getCredentials().toString());
        return ResponseEntity.ok().body(reservationService.getPayedReservations(user.get(0).getId().toString()));
    }
